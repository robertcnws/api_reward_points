...
def create_store_product_selection_buy(request, id):
    ...
    # === CARGAS LIGERAS SOLO CON CAMPOS NECESARIOS ===
    user_reporter = None
    if user_reporter_payload:
        payload = json.loads(user_reporter_payload)
        user_reporter = LoginUser.objects(username=payload.get('username')).only('id','username','email').first()
    ...
    try:
        # Trae solo lo necesario para cálculos
        reward_points = RewardPoints.objects(user=user_buyer).only(
            'id','total_gained_points','total_assigned_points','total_spent_points', 'user'
        ).first()
        if not reward_points:
            logger.error("Reward points not found for the user")
            return Response({'error': 'Reward points not found for the user'}, status=404)
        ...
        # 1) Verifica puntos UNA sola vez con costo total
        unit_points = store_product.assigned_points
        ...
        # 2) Construye selections en memoria → insert bulk
        selections = [
            RewardStoreProductSelection(
                store_product=store_product,
                user=user_buyer,
        ...
        # 3) Calcula fraction UNA vez (mismo resultado) y reutiliza
        purchase_type, purchase_fraction = calculate_purchase_fraction(
            logger,
            reward_points,
            gained_points,
            assigned_points,
            unit_points,  # por unidad
        )
        if not isinstance(purchase_type, str):
            logger.error("Error calculating redeemed order fraction")
            return purchase_type

        # 4) Prepara buys e histories en listas (sin transformaciones pesadas)
        #    – tracking_info mínimo para no desreferenciar todo el documento
        list_buys = []
        list_history = []

        # Genera números sólo una vez por buy (obvio), pero evita trabajo caro alrededor
        for sel in selections:
            order_no = generate_order_number()
            conf_no = generate_confirmation_number()
            ...
            # tracking_info ligero (evita transform_data_to_mongo)
            tracking_info = {
                'buyId': str(buy.id),  # se rellena al insertar pero sirve como placeholder
                'selectionId': str(sel.id),
                'storeProductId': str(store_product.id),
                'storeProductName': store_product.name,
                'unitPoints': unit_points,
                'quantity': default_qty,
                'createdTime': now_naive.isoformat(),
            }

            description = f'You have used {unit_points} points to redeem {default_qty} {store_product.name}'
            hist = RewardPointsHistory(
                created_time=now,
                reward_points=reward_points,
                action='spent',
                gained_points=0,
                spent_points=unit_points,
                description=description,
                info=tracking_info,
            )
            list_history.append(hist)

        # 5) Actualiza reward_points una sola vez
        reward_points.total_spent_points = (reward_points.total_spent_points or 0) + total_cost_points
        reward_points.last_modified_time = now
        reward_points.save()  # 1 write

        # 6) Notificación global (igual que tenías)
        info = f'has redeemed {len(list_buys)} orders of {store_product.name.upper()} and total points {total_cost_points}'
        ...
        # 7) Marca carts en un solo update_many server-side (MUY barato)
        RewardStoreProductSelectionCart.objects(
            store_product_selection__in=selection_ids, is_bought=False
        ).update(
            set__is_bought=True, set__last_modified_time=now
        )

        # 8) Bulk inserts (rápidos). Si prefieres señales, mantén insert(); signals de MongoEngine no se disparan en bulk,
        #    pero tus websockets ya los manejas por signals en saves normales (esto evita loops de .save()).
        bulk_save(list_buys)      # tu helper
        bulk_save(list_history)   # tu helper
        ...
        # 10) Tracking async mínimo por compra (ya sin transform pesado)
        for buy in list_buys:
            task_create_tracking_async.delay(
                ...
                tracking_info={'buyId': str(buy.id), 'storeProductId': str(store_product.id)}
            )
