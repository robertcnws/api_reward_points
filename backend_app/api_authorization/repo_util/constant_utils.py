USER_NOT_FOUND = {
    'error': 'User not found',
    'description': 'User Not Found',
    'error_name': 'user_not_found'
}
USER_NOT_FOUND_OR_INACTIVE = {
    'error': 'User not found or inactive',
    'description': 'User not found or inactive',
    'error_name': 'user_not_found'
}
USER_NOT_EXIST = {
    'error': 'User does not exist',
    'description': 'User does not exist',
    'error_name': 'user_not_exist'
}
USERNAME_REQUIRED = {
    'error': 'Username required',
    'description': 'Username is required to proceed'
}
USER_NOT_VERIFIED = {
    'error': 'User not verified',
    'description': 'User is not verified',
    'error_name': 'user_not_verified'
}
USER_NOT_APPROVED = {
    'error': 'User not approved',
    'description': 'User is not approved by admin',
    'error_name': 'user_not_approved'
}
USER_NOT_ACTIVE = {
    'error': 'User not active',
    'description': 'User is not active',
    'error_name': 'user_not_active'
}
USERNAME_AND_PASSWORD_REQUIRED = {
    'error': 'Username and password required',
    'description': 'Username and password are required to proceed',
    'error_name': 'username_and_password_required'
}
USERNAME_EXISTS_AND_IS_ACTIVE = {
    'error': 'Username already exists and is active', 
    'description': 'Username already exists and is active', 
    'error_name': 'username_exists',
}
USERNAME_EXISTS_AND_IS_NOT_VERIFIED = {
    'error': 'Username already exists and is NOT verified', 
    'description': 'Username already exists and is NOT verified',
    'error_name': 'username_not_verified',
}
COMPANY_EXISTS_AND_IS_ACTIVE = {
    'error': 'Company already exists and is active', 
    'description': 'Company already exists and is active', 
    'error_name': 'company_exists',
}
EMAIL_NOT_REGISTERED_IN_ZOHO = {
    'error': 'Email not registered in Zoho customers', 
    'description': 'Email not registered in Zoho customers', 
    'error_name': 'email_not_registered_in_zoho',
}


ERROR_VERIFICATION_CODE_EXPIRED = {
    'error': 'Expired verification code',
    'description': 'Verification code has expired, please request a new one',
    'error_name': 'verification_code_expired',
}

ERROR_VERIFICATION_CODE_INVALID = {
    'error': 'Invalid verification code',
    'description': 'Verification code is invalid',
    'error_name': 'verification_code_invalid',
}

ERROR_CONFIRMATION_CODE_REQUIRED = {
    'error': 'Confirmation code required', 
    'description': 'Confirmation code is required',
    'error_name': 'confirmation_code_required'
}

ERROR_CONFIRMATION_CODE_INVALID_OR_EXPIRED = {
    'error': 'Invalid or expired confirmation code',
    'description': 'Invalid or expired confirmation code',
    'error_name': 'invalid_confirmation_code',
}

ERROR_NEW_PASSWORD_REQUIRED = {
    'error': 'New password required',
    'description': 'New password is required',
    'error_name': 'new_password_required'
}




ERROR_INVALID_JSON = {
    'error': 'Invalid JSON',
    'description': 'Request is not in a valid format',
    'error_name': 'invalid_json'
}
ERROR_METHOD_NOT_ALLOWED = {
    'error': 'Method not allowed',
    'description': 'Method not allowed',
    'error_name': 'method_not_allowed'
}
ERROR_INVALID_CREDENTIALS = {
    'error': 'Invalid credentials',
    'description': 'Incorrect Password',
    'error_name': 'invalid_credentials'
}
ERROR_INVALID_USERNAME = {
    'error': 'Invalid credentials',
    'description': 'Username does not exist',
    'error_name': 'invalid_username'
}
ERROR_UNEXPECTED = {
    'error': 'Unexpected error', 
    'description': 'Please, try again later.',
    'error_name': 'unexpected_error'
}
ERROR_EMAIL_REQUIRED = {
    'error': 'Email required',
    'description': 'Email is required to proceed',
    'error_name': 'email_required'
}
ERROR_USERNAME_REQUIRED = {
    'error': 'Username required',
    'description': 'Username is required to proceed',
    'error_name': 'username_required'
}