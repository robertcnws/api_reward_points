...
import { EcommerceRewardPointsHistoryList } from 'src/sections/overview/e-commerce/ecommerce-reward-points-history-list';
...
export function PurchaseOverviewClientView({
  ...
  const router = useRouter();
    ...
    const displayCompanyName = useMemo(() => client?.companyName, [client]);
                  ...
                  <Box sx={{
                    ...
                    {!isMobile && (
                      <Typography variant="caption" sx={{ opacity: 0.8, mt: 1 }}>
                        Last login: <b>{fDateTime(client?.lastLogin) || 'N/A'}</b>
                      </Typography>
                    )}
                  </Box>
                }
