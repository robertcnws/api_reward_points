...
export function ItemFolderItem({
  sx,
  folder,
  selected,
  onSelect,
  ...
  const favorite = useBoolean(folder?.canBeBought);

  const [folderName, setFolderName] = useState(folder?.item?.name);

  const [values, setValue] = useState({
    quantity: 1,
    available: folder?.item?.stock_on_hand || 0,
  });
